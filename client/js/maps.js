...
let tileset = [];
let loadTileset = async function() {
            ...
            if (this.status >= 200 && this.status < 400) {
                ...
                for (let i = 0; i < data.tiles.length; i++) {
                    ...
                    if (data.tiles[i].animation != null) {
                        ...
                        for (let j = 0; j < data.tiles[i].animation.length; j++) {
                            animation.ids.push(data.tiles[i].animation[j].tileid);
                            animation.timings.push(data.tiles[i].animation[j].duration);
                        }
                        let tileCanvas = createCanvas(animation.ids.length * 16, 16);
                        let tileCtx = tileCanvas.getContext("2d");
                        resetCanvas(tileCtx);
                        for (let j in animation.ids) {
                            let tileId = animation.ids[j];
                ...
                // let raw = parser.parseFromString(this.response, "text/xml");
                ...
                //                 let animation = raw.documentElement.children[i].children[j];
                //                 let tile = {
                //                     tileIds: [],
                //                     tileTimings: [],
                //                     tileImage: new Image(),
                ...
                //                     }
                //                 }
                //                 let tileCanvas = createCanvas(tile.tileIds.length * 16, 16);
                //                 let tileCtx = tileCanvas.getContext("2d");
                //                 resetCanvas(tileCtx);
                //                 for (let k in tile.tileIds) {
                //                     let tileId = tile.tileIds[k];
...
let maps = [];
let loadMap = async function(name, index) {
    await new Promise(async function(resolve, reject) {
        let request = new XMLHttpRequest();
            ...
            if (this.status >= 200 && this.status < 400) {
                ...
                for (let i in data.layers) {
                    if (data.layers[i].name.startsWith("Darkness:")) {
                        ...
                        for (let j in data.layers[i].chunks) {
                            let chunk = data.layers[i].chunks[j];
                            for (let k in chunk.data) {
                                if (chunk.data[k] != 0) {
                                    ...
                                    let x = (k % chunk.width + chunk.x + (data.layers[i].offsetx ?? 0) / 16) * TILE_SIZE + TILE_SIZE / 2;
                                    let y = (Math.floor(k / chunk.width) + chunk.y + (data.layers[i].offsety ?? 0) / 16) * TILE_SIZE + TILE_SIZE / 2;
                                    if (array.length == 3) {
                        ...
                        let layer = data.layers[i].name.includes(":") ? Number(data.layers[i].name.split(":")[1]) : 0;
                        if (isNaN(layer)) {
                            layer = 0;
                        }
                        ...
                        if (maps[index].collisions[layer] == null) {
                            maps[index].collisions[layer] = [];
                        }
                        for (let j = 0; j < data.layers[i].chunks.length; j++) {
                            let chunk = data.layers[i].chunks[j];
                            for (let k = 0; k < chunk.data.length; k++) {
                                ...
                                for (let l = 0; l < tileset[id].collisions.length; l++) {
                                    ...
                                    for (let y = minGridY; y < maxGridY; y++) {
                                        for (let x = minGridX; x < maxGridX; x++) {
                                            ...
                                            if (maps[index].collisions[layer][y] == null) {
                                                maps[index].collisions[layer][y] = [];
                                            }
                                            if (maps[index].collisions[layer][y][x] == null) {
                                                maps[index].collisions[layer][y][x] = [];
                                            }
                                            maps[index].collisions[layer][y][x].push(collision);
                                        }
                                    }
                                }
                            }
...
let updateRenderedChunks = async function() {
    ...
    for (let i in maps[selfMap].canvases) {
        for (let j in maps[selfMap].canvases[i]) {
    ...
    for (let i = selfPlayer.chunkY - settings.renderDistance; i <= selfPlayer.chunkY + settings.renderDistance; i++) {
        for (let j = selfPlayer.chunkX - settings.renderDistance; j <= selfPlayer.chunkX + settings.renderDistance; j++) {
...
let renderChunk = function(x, y) {
    let chunkCanvas = [];
    let chunkCtx = [];
    for (let i in maps[selfMap].data[y][x]) {
        ...
        for (let j in maps[selfMap].data[y][x][i]) {
            for (let k in maps[selfMap].data[y][x][i][j].data) {
                let tileId = maps[selfMap].data[y][x][i][j].data[k] - 1;
                if (tileId != -1) {
                    let tileX = k % 16;
                    let tileY = Math.floor(k / 16);
    ...
    if (maps[selfMap].lights[y] != null && maps[selfMap].lights[y][x] != null) {
        for (let i in maps[selfMap].lights[y][x]) {
    ...
    if (maps[selfMap].particleGenerators[y] != null && maps[selfMap].particleGenerators[y][x] != null) {
        for (let i in maps[selfMap].particleGenerators[y][x]) {
...
let regions = [];
let loadRegions = async function() {
    await new Promise(async function(resolve, reject) {
        let request = new XMLHttpRequest();
