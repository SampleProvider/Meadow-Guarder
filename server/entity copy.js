...
Entity.move = function(entity, slide) {
    ...
    if (max != 0) {
        ...
        for (var i = 0; i < max; i += 1) {
            ...
            if (Entity.collisionMapEffects(entity)) {
                break;
            }
...
Entity.collideWithMap = function(entity, slide) {
    if (collisions[entity.map] == null) {
        return false;
    }
    if (collisions[entity.map][entity.layer] == null) {
    ...
    for (var y = Math.floor((entity.y - entity.height / 2) / TILE_SIZE); y < Math.ceil((entity.y + entity.height / 2) / TILE_SIZE); y++) {
        ...
        for (var x = Math.floor((entity.x - entity.width / 2) / TILE_SIZE); x < Math.ceil((entity.x + entity.width / 2) / TILE_SIZE); x++) {
            ...
            for (let i in collisions[entity.map][entity.layer][y][x]) {
                ...
                if (entity.x - entity.width / 2 < x * TILE_SIZE + collision.x + collision.width && entity.x + entity.width / 2 > x * TILE_SIZE + collision.x && entity.y - entity.height / 2 < y * TILE_SIZE + collision.y + collision.height && entity.y + entity.height / 2 > y * TILE_SIZE + collision.y) {
                    distanceX = Math.max(distanceX, (collision.x + collision.width - (entity.x - entity.width / 2)) / entity.speedX);
                    distanceY = Math.max(distanceY, (collision.y + collision.height - (entity.y - entity.height / 2)) / entity.speedY);
                }
            }
        }
    }
...
Entity.collisionMapEffects = function(entity) {
    ...
    if (regions[entity.map] != null && regions[entity.map][entity.gridY] != null && entity.region != regions[entity.map][entity.gridY][entity.gridX]) {
        ...
        entity.safeRegion = safeRegion[entity.region];
        if (entity.type == PLAYER) {
            entity.socket.emit("region", entity.region ?? WILDERNESS);
        }
        else if (entity.type == MONSTER) {
            if (entity.safeRegion) {
    ...
    if (rig.teleporting) {
        if (rig.type == PLAYER) {
            ...
            if (rig.teleportTime > 0) {
                ...
                rig.teleportTime = Math.max(rig.teleportTime - 1, -10);
            }
        }
        else {
            if (rig.teleportTime == 10) {
...
Rig.pathfind = function(rig, x, y) {
    ...
    for (var i = top; i < bottom; i++) {
        for (var j = left; j < right; j++) {
            ...
            else if (rig.type == MONSTER && regions[rig.map] != null && regions[rig.map][rig.layer] != null && regions[rig.map][rig.layer][i] != null && safeRegion[regions[rig.map][rig.layer][i][j]]) {
                grid.setWalkableAt(j - left, i - top, false);
            }
        }
    }
...
Rig.retreat = function(rig, x, y) {
    ...
    for (var i = top; i < bottom; i++) {
        for (var j = left; j < right; j++) {
            ...
            if (rig.type == MONSTER && regions[rig.map] != null && regions[rig.map][rig.layer] != null && regions[rig.map][rig.layer][i] != null && safeRegion[regions[rig.map][rig.layer][i][j]]) {
                continue;
            }
...
Rig.dodgeProjectiles = function(rig) {
    ...
    for (var i = 0; i < ENV.dodgeProjectileSearchLength; i++) {
        ...
        for (var i in projectiles) {
            ...
            if (Entity.collisionEntity(rig, projectiles[i])) {
                if (!indexChanged) {
...
Rig.contactEvents = [
    {
        ...
        event: function(rig1, rig2, data) {
            Rig.areaEffect(rig2.x, rig2.y, rig2.map, data.diameter, rig2.type, function(rig) {
                ...
                if (rig.safeRegion || rig.hp == 0 || rig.teleporting || rig.loading || rig.dialogue != null) {
                    return;
                }
...
Player.init = function(player) {
    ...
    player.socket.on("ping", function(callback) {
        callback();
    });
    player.socket.on("renderDistance", function(data) {
        ...
        player.renderDistance = data;
    });
    player.socket.on("particles", function(data) {
        if (typeof data != "boolean") {
            player.leave();
            return;
        }
        player.particles = data;
    });
    player.socket.on("cameraEffects", function(data) {
        if (typeof data != "boolean") {
            player.leave();
            return;
        }
        player.cameraEffects = data;
    });
    player.socket.on("debug", function(data) {
        if (typeof data != "boolean") {
            player.leave();
            return;
        }
...
Player.updateCollisions = function(player) {
    if (player.safeRegion || player.hp == 0 || player.teleporting || player.loading || player.dialogue != null) {
        return;
    }
    if (ENV.playerFriendlyFire) {
        Entity.searchHitboxChunks(Player.chunks, player.x, player.y, player.width, player.height, player.map, function(rig) {
            ...
            if (Entity.collisionEntity(player, rig)) {
    ...
    Entity.searchHitboxChunks(Monster.chunks, player.x, player.y, player.width, player.height, player.map, function(rig) {
        ...
        if (rig.safeRegion || rig.hp == 0 || rig.teleporting) {
            return;
        }
        if (Entity.collisionEntity(player, rig)) {
            Rig.onDamage(rig, player, DAMAGE_CONTACT);
...
Monster.updateAI = function(monster) {
    ...
    if (monster.aiState == ATTACK) {
        if (monster.target == null || monster.target.safeRegion || monster.target.hp == 0 || monster.target.teleporting || monster.target.loading || monster.target.dialogue != null || (!monster.provoked && Entity.getDistance(monster, monster.target) > monster.aggroMaxDistance)) {
            monster.target = null;
            monster.aiState = IDLE;
            monster.provoked = false;
...
Monster.updateCollisions = function(monster) {
    if (monster.safeRegion || monster.hp == 0 || monster.teleporting) {
        return;
    }
    Entity.searchHitboxChunks(Player.chunks, monster.x, monster.y, monster.width, monster.height, monster.map, function(rig) {
        ...
        if (Entity.collisionEntity(monster, rig)) {
